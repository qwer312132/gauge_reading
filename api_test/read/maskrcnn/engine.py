...
def my_new_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    print("loss")
    print(mask_logits.size())
    print(proposals[0].size())
    print(gt_masks[0].size())
    print(gt_labels[0].size())
    print(mask_matched_idxs[0].size())
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq):
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #     print(loss)
        # print(images[0].size())
        # cv2.imshow("qwer",images[0].detach().numpy())
        # print(type(images[0]))
